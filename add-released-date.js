...
async function addReleasedDateColumn() {
  try {
    ...
    // Add released_date column to each record (empty by default)
    const updatedRecords = records.map(record => {
      if (!record.released_date) {
        record.released_date = '' // Add empty released_date field
      }
      return record
    })
    ...
    const columns = [
      ...
      'product_identifier', 'product_identifier_type', 'brand', 'cost', 'status', 'drop_date', 'released_date'
    ]
    ...
    console.log('CSV file updated successfully with released_date column!')
